...
<template>
  <div id="merchant-info">
      ...
      <div>
        ...
        <b-form>
          <b-form-group>
            ...
            <b-form-input
              id="merchant-info-name"
              v-model.trim="merchantInfo.name"
              :disabled="!editable"
            ></b-form-input>
          </b-form-group>
          <b-form-group>
          ...
          <b-form-group>
            ...
            <b-form-input
              id="merchant-address-x"
              v-model.trim="merchantInfo.address.longitude"
              :disabled="!editable"
            ></b-form-input>
            <label for="merchant-address-y">Y: </label>
            <b-form-input
              id="merchant-address-y"
              v-model.trim="merchantInfo.address.latitude"
              :disabled="!editable"
            ></b-form-input>
            <label for="merchant-address-note">Note: </label>
            <b-form-input
              id="merchant-address-note"
              v-model.trim="merchantInfo.address.note"
              :disabled="!editable"
            ></b-form-input>
          </b-form-group>
          <b-form-group>
            <b-button variant="outline-dark" @click="enter_edit_mode" :disabled="editable"
              >Edit</b-button
            >
            <b-button variant="outline-success" @click="modify_info" :disabled="!editable"
              >Submit</b-button
            >
          </b-form-group>
        </b-form>
    ...
    <div id="merchant-right-blank"></div>
  </div>
</template>
  ...
  data() {
    return {
      vertificationType: [
        { enum: "", name: "" },
        {
          enum: "JUST_REGISTERED",
          name: "ToBeAdded",
          msg: "You just registered, change the default info and submit for approval."
        },
        {
          enum: "BEFORE_APPROVAL",
          name: "UnderReview",
          msg: "Your info submitted, waiting for the manager to check..."
        },
        {
          enum: "GET_DISAPPROVAL",
          name: "Disapproval",
          msg: "Your info submitted, but gotta rejected. Please modify and submit again."
        },
        {
          enum: "AFTER_VERTIFICATION",
          name: "verified",
          msg: "You gotta approved! Manage your menu and make deals!"
        }
      ],
      merchantType: [
        { enum: "", name: "" },
  ...
  computed: {
    ...
    get_state() {
      for (let type of this.vertificationType) {
        if (this.merchantInfo.verEnum === type.enum) return type;
      }
      return null;
    },
    type_options() {
