      ...
      <div id="packs-list">
        ...
        <div class="menu-item" v-for="(pack, index) of packs" :key="'pack_' + index">
          ...
          <p>
            ...
            <b>{{ pack.price }}</b>
            <label>Deadline:</label>
            <code>{{ pack.deadline }}</code>
          </p>
          <b-card>
            <p class="card-text">
              Pack Info
              <b-button variant="outline-info" v-b-toggle="'collapse_inner_' + index">
                +
              </b-button>
            </p>
            <b-collapse :id="'collapse_inner_' + index" class="mt-2">
              <b-card>
                <b-card-text v-for="(dish, i) of pack.dishes" :key="'pack_' + index + '_dish_' + i">
                  {{ dish.dish.name }} * {{ dish.num }}
                </b-card-text>
    ...
    <div id="merchant-right-blank">
      <b-form id="new-dish-form">
        ...
        <b-form-group class="new-group">
          <label>Name:</label>
          <b-form-input class="w2" v-model="newDish.name" />
        </b-form-group>
        <b-form-group class="new-group">
          <label>Type:</label>
        ...
        <b-form-group class="new-group">
          <label>Price:</label>
          <b-form-input class="w1" v-model="newDish.price" />
          <label>Supply:</label>
          <b-form-input class="w1" v-model="newDish.supply" />
        </b-form-group>
        <b-form-group class="new-group">
          <label>Deadline:</label>
          <b-form-input class="w3" v-model="newDish.deadline" type="date" />
        </b-form-group>
        <b-button variant="outline-success" @click="add_dish">
          Add
        </b-button>
      </b-form>
      <b-form id="new-pack-form">
        ...
        <b-form-group class="new-group">
          <label>Name:</label>
          <b-form-input class="w2" v-model="newPack.name" />
          <label>Price:</label>
          <b-form-input class="w1" v-model="newPack.price" />
        </b-form-group>
        <b-form-group class="new-group">
          <label>Deadline:</label>
          <b-form-input class="w3" v-model="newPack.deadline" type="date" />
        </b-form-group>
        <b-form-group class="new-group">
          <h5>AddDishInPack</h5>
          <b-form-select class="w2" v-model="newDishInPack.dish" :options="dish_options" />
          <b-form-input class="w1" v-model="newDishInPack.num" />
          <b-button variant="success" @click="add_select">
            +
          </b-button>
        </b-form-group>
        <b-form-group class="new-group">
          ...
          <div class="dishes-in-pack">
            <b-card>
              <b-card-text v-for="(dish, i) of newPack.dishes" :key="'dish_in_pack_' + i">
                {{ dish.dish.name }} * {{ dish.num }}
              </b-card-text>
            </b-card>
          </div>
        </b-form-group>
        <b-button variant="outline-success" @click="add_pack">
          Add
        </b-button>
      </b-form>
    </div>
  ...
  methods: {
    ...
    add_select() {
      if (this.newDishInPack.dish !== null && this.newDishInPack.num > 0) {
        ...
        if (sign === 1)
          this.newPack.dishes.push({
            dish: this.dishes[this.newDishInPack.dish],
            num: this.newDishInPack.num
          });
      }
    }
  }
...
#merchant-right-main::-webkit-scrollbar {
  /*滚动条整体样式*/
