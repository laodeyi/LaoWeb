      ...
      <div id="merchant-deal-list">
        <b-card id="deal-card" no-body>
          <b-tabs pills card vertical>
            <b-tab active>
              <template slot="title">
                <b-spinner variant="success" small />
                {{ dealType[1].name }}
              </template>
              <div class="list-wrap-div">
                <div v-for="(deal, index) of deals.paid" class="insert" :key="'cancelled_' + index">
                  <b>
                    MemId: {{ deal.memId }}>
                    <code>{{ deal.setupTime }}</code>
                  </b>
                  <p>
                    To {{ deal.memberAddress.note }} | Sum: {{ deal.finalPrice }} | Delivering ...
                  </p>
                </div>
              </div>
            </b-tab>
      ...
      utils.axiosMethod({
        ...
        callback: response => {
          if (response.data.code === 1) {
            if (i === 2) this.deals.cancelled = response.data.list;
            else if (i === 1) this.deals.paid = response.data.list;
            else if (i === 3) this.deals.refund = response.data.list;
            else if (i === 4) this.deals.complete = response.data.list;
