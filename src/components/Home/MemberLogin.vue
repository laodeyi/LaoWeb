...
<template>
  <div id="member-login">
    <b-form id="member-login-form">
      ...
      <b-form class="member-group" inline>
        <label for="member-email-pre">Email:</label>
        <b-form-input
          v-model.trim="logData.email_pre"
          id="member-email-pre"
          placeholder="member123"
        ></b-form-input>
        <b-input-group prepend="@" id="member-email-post-group">
          <b-form-input
            v-model.trim="logData.email_post"
            id="member-email-post"
            placeholder="xxx.com"
          ></b-form-input>
        </b-input-group>
      </b-form>
      ...
      <b-form-group class="member-group">
        ...
        <b-form-input
          ...
          v-model.trim="logData.password"
        ></b-form-input>
      </b-form-group>
      <b-form-group v-if="log_type === 'register'" class="member-group">
        <label for="member-vertification">Vertification Code:</label>
        <b-form-input
          id="member-vertification"
          name="vertification"
          type="password"
          variant="outline-primary"
          v-model.trim="logData.vertification"
        ></b-form-input>
        <b-button
          :disabled="countData.dismissCountDown !== 0"
          id="member-email-button"
          variant="outline-info"
          @click="sendCode"
          >Send</b-button
        >
        <b-alert
          id="member-email-alert"
        ...
        >
          ...
          <b-progress
            variant="warning"
            :max="countData.dismissSecs"
            :value="countData.dismissCountDown"
            height="4px"
          />
        </b-alert>
      </b-form-group>
      ...
      <b-form-group class="member-group" id="member-button-group">
        <b-button v-if="log_type === 'login'" variant="outline-dark" @click="member_login"
          >Login</b-button
        >
        <b-button v-else="log_type === 'register'" variant="outline-dark" @click="member_register"
          >Register</b-button
        >
      </b-form-group>
    </b-form>
...
export default {
  ...
  computed: {
    email: function() {
      if (this.logData.email_pre === "") return "member123@xxx.com";
      else return this.logData.email_pre + "@" + this.logData.email_post;
    },
    log_type: function() {
  ...
  methods: {
      ...
      utils.axiosMethod({
        ...
        callback: response => {
          ...
          if (response.data.code === 1) {
            ...
            this.$router.push({
              name: "memberMain",
              params: { mem_name: response.data.object.name.toString() }
            });
          }
        }
      });
    ...
    member_register() {
      ...
      } else alert("Not correct vertification code!");
    },
