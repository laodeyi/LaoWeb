  ...
  methods: {
    get_ratio() {
      utils.axiosMethod({
        ...
        callback: response => {
          if (response.data.code === 1) this.ratio = response.data.object;
        }
      });
    },
    get_profits() {
      utils.axiosMethod({
        ...
        params: { type: "all" },
        callback: response => {
          if (response.data.code === 1) this.profits = response.data.list;
        }
      });
    },
    save_ratio() {
    ...
    get_discounts() {
      utils.axiosMethod({
        ...
        callback: response => {
          if (response.data.code === 1) this.discounts = response.data.list;
        }
      });
    },
    save_discounts() {
      utils.axiosMethod({
        ...
        callback: response => {
          alert(response.data.msg);
          if (response.data.code === 1) this.get_discounts();
        }
      });
    }
  }
