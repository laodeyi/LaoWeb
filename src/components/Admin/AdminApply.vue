        ...
        >
          ...
          <b-list-group flush>
            ...
            <b-list-group-item>
              <label>Address: </label>
              <b
                >X: <code>{{ apply.address.longitude }}</code
                >,</b
              >
              <b
                >Y: <code>{{ apply.address.latitude }}</code
                >,</b
              >
              <b
                >Note: <code>{{ apply.address.note }}</code></b
              >
            </b-list-group-item>
          </b-list-group>
  ...
  methods: {
    get_applies() {
      utils.axiosMethod({
        ...
        callback: response => {
          if (response.data.code === 1) this.applies = response.data.list;
        }
      });
    },
    reject_apply(i) {
      utils.axiosMethod({
        ...
        callback: response => {
          ...
          if (response.data.code === 1) this.get_applies();
        }
      });
    },
    approve_apply(i) {
