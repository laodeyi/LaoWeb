...
<template>
  <div id="admin-stat">
    <div id="admin-left-nav"></div>
    <div id="admin-right-main">
      <div id="stat-info">
        <h2>
          Current Profits:<b>{{ profit }}</b>
        </h2>
      </div>
      <div id="stat-chart">
        <div id="member-chart">
          <doughnut-chart
            ...
            :chart-data="member_val_data"
          ></doughnut-chart>
          <bar-chart
            v-if="loaded"
            id="member-level-chart"
            class="bar-chart"
            color-index="2"
            :labels="member_lv_data.labels"
            :label="member_lv_data.label"
            :data="member_lv_data.data"
          ></bar-chart>
        </div>
        <div id="merchant-chart">
          <doughnut-chart
            v-if="loaded"
            id="merchant-ver-chart"
            class="doughnut-chart"
            :chart-data="merchant_ver_data"
          ></doughnut-chart>
          <bar-chart
            v-if="loaded"
            id="merchant-type-chart"
            class="bar-chart"
            color-index="4"
            :labels="merchant_type_data.labels"
            :label="merchant_type_data.label"
            :data="merchant_type_data.data"
          ></bar-chart>
        </div>
        <div id="deal-chart">
          <pie-chart
            ...
            id="deal-state-chart"
            class="pie-chart"
            :chart-data="deal_state_data"
          ></pie-chart>
        </div>
      </div>
    </div>
    <div id="admin-right-blank"></div>
  </div>
</template>
  ...
  methods: {
    ...
    get_members_by_lv(i) {
      utils.axiosMethod({
        ...
        callback: response => {
          if (response.data.code === 1) this.member_lv_data.data[i - 1] = response.data.list.length;
        }
      });
    },
    ...
    get_profit() {
      utils.axiosMethod({
        ...
        callback: response => {
          if (response.data.code === 1) this.profit = response.data.object;
        }
      });
    }
  }
