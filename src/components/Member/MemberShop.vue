    ...
    <div id="member-left-nav">
      <b-list-group v-b-scrollspy.member-right-main>
        <b-list-group-item
          ...
          disabled="disabled"
          >{{ dishType[i].name }}</b-list-group-item
        >
        <b-list-group-item variant="danger" href="#dish_type_5" disabled="disabled">{{
          dishType[5].name
        }}</b-list-group-item>
      </b-list-group>
      <b-collapse id="collapse">
        <b-card v-model="toggle_info">
          ...
          <b-collapse id="collapse_inner" class="mt-2">
            <b-card>
              <b-card-text v-for="(dish, index) of toggle_info.dishes" :key="index"
                >{{ dish.dish.name }} * {{ dish.num }}</b-card-text
              >
    ...
    <div id="member-right-blank">
      <div id="shop-chart" v-model="shopChart">
        ...
        <b-list-group id="chart-list">
          <b-list-group-item
            class="d-flex justify-content-between align-items-center chart-list-item"
            v-for="(dish, index) of shopChart.dishes"
            :key="'chart_dish' + index"
          >
            {{ dish.name }}
          ...
          <b-list-group-item
            class="d-flex justify-content-between align-items-center chart-list-item"
            v-for="(pack, index) of shopChart.packs"
            :key="'chart_pack' + index"
          >
            {{ pack.name }}
        ...
        <b-button
          ...
          variant="outline-success"
          v-b-modal.modal-pay
          :disabled="!chart_empty"
          >Submit</b-button
        >
        <b-modal id="modal-pay" ref="modal" title="Pay now or later." hide-footer="hide-footer">
          <form id="pay-modal-form" @submit.stop.prevent="handleSubmit">
            ...
            <b-form-group>
              <b-button variant="success" @click="pay_deal">
                Pay Now
              </b-button>
  ...
  computed: {
    ...
    addresses_options() {
      ...
      this.addresses.forEach((address, index) => {
        array.push({
          value: index,
          text: `X: ${address.longitude}, Y: ${address.latitude}  ${address.note}`
        });
      });
      return array;
    },
  ...
  methods: {
    ...
    get_name_by_enum(typeEnum) {
      for (let type of this.dishType) {
        if (type.enum === typeEnum) return type.name;
      }
    },
    get_member_discount() {
      utils.axiosMethod({
        ...
        callback: response => {
          if (response.data.code === 1) this.mem_dis = response.data.object;
        }
      });
    },
    get_member_payment() {
      ...
      new Promise(resolve => {
        ...
        utils.axiosMethod({
          ...
          callback: response => {
            ...
            if (response.data.code === 1) this.$router.push({ name: "memberDeal" });
          }
        });
      });
    },
    pay_deal() {
      new Promise(resolve => {
        utils.axiosMethod({
          ...
          data: this.get_deal_object(),
          callback: response => {
            if (response.data.code === 1) resolve(response.data.object.id);
            else alert(response.data.msg);
          }
        });
      })
            ...
            new Promise(resolve => {
              if (deal_id !== null) {
                utils.axiosMethod({
                  ...
                  callback: response => {
                    alert(response.data.msg);
                    if (response.data.code === 1) resolve(deal_id);
                    else {
