        ...
        <b-card id="deal-card" no-body>
          <b-tabs pills card vertical>
            <b-tab active>
              ...
              <b-modal id="modal-pay" ref="payModal" title="Pay now." hide-footer="hide-footer">
              ...
              <div class="list-wrap-div">
                <div
                  v-for="(deal, index) in deals.obligation"
                  class="insert"
                  :key="'obligation_' + index"
                >
                  <b
                    >MerId: {{ deal.merId }}>> <code>{{ deal.setupTime }}</code></b
                  >
                  <p>
                    To {{ deal.memberAddress.note }} | Sum: {{ deal.finalPrice }} |
                    <a class="pay-a" v-b-modal.modal-pay @click="set_current_pay_deal(index)"
                      >Pay</a
                    >
                    in 3min or
            ...
            <b-tab :title="dealType[2].name">
              <div class="list-wrap-div">
                <div
                  v-for="(deal, index) of deals.cancelled"
                  class="insert"
                ...
                >
                  ...
                  <p>
                    To {{ deal.memberAddress.note }} | Sum: {{ deal.finalPrice }} |
                    <label>Description: </label> This deal had been cancelled
                  </p>
                </div>
              </div>
            </b-tab>
            ...
            <b-tab :title="dealType[3].name">
              ...
              <div class="list-wrap-div">
                <div v-for="(deal, index) of deals.paid" class="insert" :key="'paid_' + index">
                  ...
                  >
                  <p>
                    To {{ deal.memberAddress.note }} | Sum: {{ deal.finalPrice }} | under
                    delivering... |
                    <b-button
                      variant="outline-warning"
                      v-b-modal.modal-refund
                      @click="set_current_refund_deal(index)"
                      >Refund</b-button
                    >
                    <b-button variant="outline-success" @click="complete_deal(index)"
                      >Received</b-button
                    >
                  </p>
                </div>
            ...
            <b-tab :title="dealType[4].name">
              <div class="list-wrap-div">
                <div v-for="(deal, index) of deals.refund" class="insert" :key="'refund_' + index">
                  ...
                  >
                  <p>
                    To {{ deal.memberAddress.note }} | Sum: {{ deal.finalPrice }} | Refund:
                    {{ deal.refund }} | <label>Description: </label> This deal had been refunded
                  </p>
                </div>
              </div>
            </b-tab>
            ...
            <b-tab :title="dealType[5].name">
              <div class="list-wrap-div">
                <div
                  v-for="(deal, index) of deals.complete"
                  class="insert"
                  :key="'complete_' + index"
                >
                  <b
                  ...
                  <p>
                    To {{ deal.memberAddress.note }} | Sum: {{ deal.finalPrice }} |
                    <label>Description: </label> This deal had been received
                  </p>
                </div>
              </div>
            </b-tab>
...
export default {
  ...
  created() {
    for (let i of this.typeCounter) this.get_deals(i);
      ...
      utils.axiosMethod({
        ...
        callback: response => {
          if (response.data.code === 1) {
            if (i === 1) this.deals.obligation = response.data.list;
            else if (i === 2) this.deals.cancelled = response.data.list;
            else if (i === 3) this.deals.paid = response.data.list;
            else if (i === 4) this.deals.refund = response.data.list;
            else if (i === 5) this.deals.complete = response.data.list;
