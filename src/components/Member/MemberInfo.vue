      ...
      <div id="member-info-address" class="member-info-div">
        ...
        >
          ...
          <b-form-group label="Latitude" :label-for="'latitude_' + address.id">
            <b-form-input
              :id="'latitude_' + address.id"
              type="text"
              v-model.trim="address.latitude"
              disabled="disabled"
            ></b-form-input>
          </b-form-group>
          ...
          <b-form-group>
            <b-button :id="'delete_' + address.id" @click="delete_address(address.id)"
              >Delete</b-button
            >
          </b-form-group>
        </b-form>
        ...
        <b-form id="add_address_form">
          <b-form-group
            description="0.00~99.99"
            label="Longitude"
            label-for="add_address_longitude"
          >
            <b-form-input
              id="add_address_longitude"
              type="text"
              v-model.trim="addAddress.longitude"
            ></b-form-input>
          </b-form-group>
          <b-form-group description="0.00~99.99" label="Latitude" label-for="add_address_latitude">
            <b-form-input
              id="add_address_latitude"
              type="text"
              v-model.trim="addAddress.latitude"
            ></b-form-input>
          </b-form-group>
        ...
        <b-form id="member-payment-form">
          <b-form-group>
            ...
            <b-form-input
              id="payment-account"
              type="text"
              v-model.trim="memberPayment.account"
              :disabled="hasPayment"
            ></b-form-input>
          </b-form-group>
          <b-form-group v-if="!hasPayment">
            ...
            <b-form-input
              id="payment-password"
              type="password"
              v-model.trim="memberPayment.password"
            ></b-form-input>
          </b-form-group>
          <b-form-group v-if="hasPayment">
            ...
            <b-form-input
              id="payment-balance"
              type="text"
              v-model.trim="memberPayment.balance"
              disabled="disabled"
            ></b-form-input>
          </b-form-group>
          <b-form-group>
...
export default {
  ...
  computed: {
    ...
    addressValidation() {
      return (
        this.addAddress.latitude !== undefined &&
        this.addAddress.longitude !== undefined &&
        this.addAddress.latitude !== "" &&
        this.addAddress.longitude !== ""
      );
    }
  },
  created() {
  ...
  methods: {
    ...
    edit_info() {
      if (!this.infoEditable) this.infoEditable = true;
    },
    modify_info() {
      if (this.infoEditable)
        utils.axiosMethod({
