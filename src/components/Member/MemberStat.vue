...
<template>
  <div id="member-stat">
    <div id="member-left-nav"></div>
    <div id="member-right-main">
      <div id="info-stat">
        <h2>Personal Stat</h2>
        <p>
          <label>MemberId: </label><b>{{ member_id }}</b>
        </p>
        <p>
          <label>MemberEmail: </label><b>{{ member_email }}</b>
        </p>
        <p>
          <label>MemberLevel: </label><b>{{ member_level_enum }}</b>
        </p>
        <p>
          <label>Total Consumption: </label><b>{{ consumption }}</b>
        </p>
        <p>
          <label>Favourite Merchants: </label
          ><b v-for="merchant of mostUsuallyMerchants">
    ...
    cancel_account() {
      utils.axiosMethod({
        ...
        callback: response => {
          ...
          if (response.data.code === 1) this.$router.push({ name: "memberLogin" });
        }
      });
    }
  }
